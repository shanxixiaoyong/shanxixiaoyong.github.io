...
test("billiards pins the independent physics and ball renderer before the game runtime", () => {
  ...
  assert.equal(billiards.cacheVersion, "billiards-tactile-surfaces-20260712l");
...
test("site contract rejects reordered billiards layers and stale renderer cache versions", () => {
  const renderer = '  <script src="assets/billiards-ball-renderer.js?v=billiards-tactile-surfaces-20260712l"></script>';
  const game = '  <script src="assets/billiards-love-game.js?v=billiards-tactile-surfaces-20260712l"></script>';
  ...
  assertSiteValidatorRejectsMutation("game-billiards-love.html", (source) => source.replace(
    renderer,
    renderer.replace("billiards-tactile-surfaces-20260712l", "billiards-love-stale")
  ));
});
