...
test("billiards pins the independent physics and ball renderer before the game runtime", () => {
  ...
  assert.equal(billiards.cacheVersion, "billiards-liquid-led-20260712k");
...
test("site contract rejects reordered billiards layers and stale renderer cache versions", () => {
  const renderer = '  <script src="assets/billiards-ball-renderer.js?v=billiards-liquid-led-20260712k"></script>';
  const game = '  <script src="assets/billiards-love-game.js?v=billiards-liquid-led-20260712k"></script>';
  ...
  assertSiteValidatorRejectsMutation("game-billiards-love.html", (source) => source.replace(
    renderer,
    renderer.replace("billiards-liquid-led-20260712k", "billiards-love-stale")
  ));
});
