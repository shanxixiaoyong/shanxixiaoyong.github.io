...
test("billiards pins the ball renderer between content and game while allowing the parallel file to land later", () => {
  ...
  assert.equal(billiards.cacheVersion, "billiards-love-physics-theatre-20260711f");
...
test("site contract rejects reordered billiards layers and stale renderer cache versions", () => {
  const renderer = '  <script src="assets/billiards-ball-renderer.js?v=billiards-love-physics-theatre-20260711f"></script>';
  const game = '  <script src="assets/billiards-love-game.js?v=billiards-love-physics-theatre-20260711f"></script>';
  ...
  assertSiteValidatorRejectsMutation("game-billiards-love.html", (source) => source.replace(
    renderer,
    renderer.replace("20260711f", "20260711-stale")
  ));
});
