...
test("billiards pins the independent physics and ball renderer before the game runtime", () => {
  ...
  assert.equal(billiards.cacheVersion, "billiards-love-smooth-result-20260712d");
...
test("site contract rejects reordered billiards layers and stale renderer cache versions", () => {
  const renderer = '  <script src="assets/billiards-ball-renderer.js?v=billiards-love-smooth-result-20260712d"></script>';
  const game = '  <script src="assets/billiards-love-game.js?v=billiards-love-smooth-result-20260712d"></script>';
  ...
  assertSiteValidatorRejectsMutation("game-billiards-love.html", (source) => source.replace(
    renderer,
    renderer.replace("billiards-love-smooth-result-20260712d", "billiards-love-stale")
  ));
});
