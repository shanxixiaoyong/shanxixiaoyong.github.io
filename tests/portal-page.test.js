...
test("homepage exposes five distinct full-bleed destinations", () => {
  ...
  for (const title of ["心动2048", "心动桌球"]) {
...
test("desktop portal uses a focused five-destination matrix", () => {
  assert.match(css, /@media \(min-width: 768px\)/);
  assert.match(css, /grid-template-columns: repeat\(16, minmax\(0, 1fr\)\)/);
  assert.match(css, /\.portal-academic \{[\s\S]*?grid-column: 1 \/ 7;[\s\S]*?grid-row: 1 \/ 4;/);
  assert.match(css, /\.portal-knowledge \{[\s\S]*?grid-column: 7 \/ 12;[\s\S]*?grid-row: 1;/);
  assert.match(css, /\.portal-tools \{[\s\S]*?grid-column: 12 \/ 17;[\s\S]*?grid-row: 1;/);
  assert.match(css, /\.portal-game \{[\s\S]*?grid-column: 7 \/ 12;[\s\S]*?grid-row: 2 \/ 4;/);
  assert.match(css, /\.portal-billiards \{[\s\S]*?grid-column: 12 \/ 17;[\s\S]*?grid-row: 2 \/ 4;/);
});
